...
async def handle_contact(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get('state') != 'WAIT_ORDER':
        ...
        # Если это запрос фото (как на твоем скриншоте)
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if msg == "📸 Получить фото заказа":
        ...
        # Если номер уже есть, не просим кнопку, а сразу шлем запрос
...
async def query_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data.startswith("st_"):
        ...
        text = "⏳ Ваш заказ в работе! Менеджер скоро свяжется с вами." if "work" in query.data else "❌ Заказ на этот номер не найден."
...
async def photo_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message.from_user.id == ADMIN_ID and update.message.reply_to_message:
        try:
            ...
            await context.bot.send_photo(chat_id=target_id, photo=update.message.photo[-1].file_id, caption="📸 Ваш заказ готов! Вот фото.")
